...
public class SortiesAdminController {
    ...
    private void setupFilterCombo() {
        ...
        filterCombo.valueProperty().addListener((obs, o, n) -> renderCards(getViewList()));
    }
    ...
    private void setupSortControls() {
        if (sortCombo != null) {
            ...
            sortCombo.valueProperty().addListener((obs, o, n) -> renderCards(getViewList()));
        }

        if (sortDirBtn != null) {
            // Par défaut: date desc (récent -> ancien)
    ...
    private List<AnnonceSortie> getViewList() {
        ...
        Comparator<AnnonceSortie> cmp = switch (key) {
            ...
            default -> Comparator.comparing(AnnonceSortie::getDateSortie, Comparator.nullsLast(Comparator.naturalOrder()));
        };
    ...
    private Node createCard(AnnonceSortie a) {
        ...
        HBox actions = new HBox(10, quickEdit, quickParts, quickDelete);
    ...
    private void openParticipations(AnnonceSortie a) {
        ...
        List<ParticipationSortie> pending = all.stream()
                .filter(p -> "EN_ATTENTE".equalsIgnoreCase(safe(p.getStatut()).trim()))
                .toList();
        ...
        if (pending.isEmpty()) {
            ...
            for (ParticipationSortie p : pending) {
                root.getChildren().add(adminRequestRow(a, p));
            }
        }
    ...
    private Node adminRequestRow(AnnonceSortie a, ParticipationSortie p) {
        VBox box = new VBox(6);
