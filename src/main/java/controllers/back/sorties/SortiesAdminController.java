...
public class SortiesAdminController {
    ...
    private void openParticipations(AnnonceSortie a) {
        ...
        Button btnDetails = new Button("📋 Détails annonce");
    ...
    private void showAnnonceDetails(AnnonceSortie a, Stage owner) {
        ...
        // Infos
        String date   = a.getDateSortie() != null ? a.getDateSortie().format(DT_FMT) : "—";
        String budget = a.getBudgetMax() <= 0 ? "Aucun budget" : MONEY_FMT.format(a.getBudgetMax()) + " TND max";

        VBox details = new VBox(8,
                detailRow("📍", "Lieu",      safe(a.getVille()) + " • " + safe(a.getLieuTexte())),
                detailRow("🤝", "Rendez-vous", safe(a.getPointRencontre())),
                detailRow("🏃", "Activité",  safe(a.getTypeActivite())),
                detailRow("📅", "Date",      date),
                detailRow("💰", "Budget",    budget),
                detailRow("👥", "Places",    a.getNbPlaces() + " place(s)")
        );
    ...
    private Node adminRequestRow(AnnonceSortie a, ParticipationSortie p, Runnable onResolved) {
        ...
        accept.setOnAction(e -> {
            try {
                ...
                showInfo("Acceptée", "La demande a été acceptée. Un email/SMS de confirmation a été envoyé.");
                box.setDisable(true);
                box.setOpacity(0.55);
        ...
        refuse.setOnAction(e -> {
            try {
                ...
                showInfo("Refusée", "La demande a été refusée. Un email/SMS de notification a été envoyé.");
                box.setDisable(true);
                box.setOpacity(0.55);
                if (onResolved != null) onResolved.run();
            } catch (Exception ex) {
