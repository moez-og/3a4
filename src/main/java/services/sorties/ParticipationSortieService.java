...
public class ParticipationSortieService implements ServiceBase {
    ...
    public boolean updateStatus(int id, String statut) {
        ...
        if (!oldStatus.isBlank() && oldStatus.equalsIgnoreCase(newStatus)) {
            return true; // no change => no duplicate notification
        }
        ...
        try (PreparedStatement ps = cnx.prepareStatement(sql)) {
            ...
            if (isAccepted || isRefused) {
                ...
                try {
                    ...
                    if (annonce != null) {
                        notifSent = emailSmsService.envoyerNotification(before, annonce, isAccepted);
                    } else {
            ...
            return !notifAttempted || notifSent;
        } catch (SQLException e) {
