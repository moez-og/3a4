...
public class FrontDashboardController implements ShellNavigator {
    ...
    public void navigate(String route) {
        ...
        if (route.startsWith(ROUTE_SORTIE_DETAILS_PREFIX)) {
            ...
            try {
                int id = Integer.parseInt(raw);
                showSortieDetails(id, false, false);
            } catch (Exception e) {
        ...
        if (route.startsWith(ROUTE_SORTIE_REQUESTS_PREFIX)) {
            ...
            try {
                int id = Integer.parseInt(raw);
                showSortieDetails(id, true, false);
            } catch (Exception e) {
                info("Navigation", "ID sortie invalide: " + raw);
            }
            return;
    ...
    private void showSortieDetails(int id, boolean openRequests, boolean openAlbum) {
        navSorties.setSelected(true);
        ...
        try {
            ...
            if (controller instanceof SortieDetailsController c) {
                ...
                if (openAlbum) c.openAlbumFromOutside();
            }
    ...
    public void showNotifications() {
        if (currentUser == null || currentUser.getId() <= 0) {
            info("Notifications", "Connecte-toi pour voir tes notifications.");
            return;
        }
    ...
    private void handleNotificationOpen(Notification n) {
        ...
        boolean openAlbum = (t == NotificationType.SORTIE_RECAP_UPDATED);

        // Open directly so we can deep-link to a specific tab.
        showSortieDetails(sortieId, openRequests, openAlbum);
    }
